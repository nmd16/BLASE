...
run_rlMCMC_fast(MCMCSeed="test", File2011,File2012, method,its,burnin,DPits, DPburnin, gap, reps, thinning, DPthinning, printgap, Comment, coefs.primary, coefs.primary.all, coefs.secondary, col.Y1, col.Y2, secondary.option, secondary.option2, ObsBlock_restriction="yes", BlockSeed_restriction="no", error.file, error.fields, blockingVars, matches, a.beta.prior, b.beta.prior, Hstar, DPOffset, Y1.name, Y2.name,paste(namepart,"Test",sep=""),threshold,complete.out,type2seeds= NULL)
...
set.seed(MCMCSeed)
  ...
  modelPart <- model.matrix( lm(formula = File2011[, 1] ~ Cmathscal + sex * ethnic + YearInd *ethnic, data = File2011))
  modelPartSecondary <-  model.matrix( lm(formula = File2011[, 1] ~ YearInd*sex + YearInd*ethnic, data = File2011))

  setdiff(colnames(modelPartSecondary),colnames(modelPart))
  
  File1_data = cbind( File1[,col.Y1], modelPart[,-c(1:2)],modelPartSecondary[,"YearIndTRUE:sexM"])
  colnames(File1_data)[col.Y1] = Y1.name

modelPart <- model.matrix( lm(formula = File2012[, 1] ~ Cmathscal + sex * ethnic + YearInd *ethnic, data = File2012))
modelPartSecondary <-  model.matrix( lm(formula = File2012[, 1] ~ YearInd*sex + YearInd*ethnic, data = File2012))


  File2_data = cbind( File2[,col.Y2], modelPart[,-c(1:2)],modelPartSecondary[,"YearIndTRUE:sexM"])
  colnames(File2_data)[col.Y2] = Y2.name
  colnames(File2_data)[ncol(File2_data)] = "YearIndTRUE:sexM"
  colnames(File1_data)[ncol(File1_data)] = "YearIndTRUE:sexM"
  

#load("N:/Simulations/DataSet/FullDataRun/GFile1Formatted.RData")
#load("N:/Simulations/DataSet/FullDataRun/GFile2Formatted.RData")
#File1_data= File1_data[,1:19]
#File2_data= File2_data[,1:19]

#center
#File2_data[,1]=File2_data[,1]-mean(File2_data[,1])
  ...
  where.coefs.secondary = c(8,2,3:7,19,14:18)
