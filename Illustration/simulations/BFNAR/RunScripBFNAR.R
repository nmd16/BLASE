...
load("N:/Simulations/LargeEthnic/MainLargeEthnic/MainEffectStart.RData")

File2012T[,"ethnic"] = File2011T[,"ethnic"]
File2012 = File2012T
File2012[in.error,"ethnic"] = File2[in.error,"ethnic"]
sum(File2012[,"ethnic"]!=File2[,"ethnic"])

sum(File2012T[,"ethnic"]!=File2011T[,"ethnic"])
sum(File2011T[,"ethnic"]!=File2011[,"ethnic"])
sum(File2012T[,"ethnic"]!=File2012[,"ethnic"]) # 11699
sum(File2012T[,"ethnic"]!=File2[,"ethnic"]) # 11721

table(File2012T[in.error,"ethnic"],File2012[in.error,"ethnic"])
table(File2012T[,"ethnic"],File2012[,"ethnic"]) # Good. 
table(File2011T[,"ethnic"],File2011[,"ethnic"]) # Good. 

setwd("N:/Simulations/DeliberateError") 

rm(File1_data,File2_data,GTHolder,Data,B12.holder,AllDays11,BDmatch,BDay2012,BInd,CMath,Diff.11.12,Diff.12.11,Diff12.11,DisagreePool,Error.Options,Model1A,ModelFit,ModelFit2,ModelNew3,ModelNew4,Race_Nmatch,Race_math,School_Nmatch,School_match,Sex_match,which.Large11,which.Large12,which.LargeBoth)
...
File2011[,1] = File2011[,1] - mean(File2011[,1])
...
# We change this part for the new run!!!!! Do that!! 
...
setdiff( which(File1_data[1:n1,"ethnicB"]==1), which(File2011[,"ethnic"]=="B"))
setdiff( which(File1_data[1:n1,"ethnicH"]==1), which(File2011[,"ethnic"]=="H"))

# %%%%%%%%%%%%%%%%%%%%%%%%%%%% ## 
##    Run the script: Gutman   ##
## %%%%%%%%%%%%%%%%%%%%%%%%%%% ## 

File1_data = File1_data[1:n1,]
File2_data = File2_data[1:n1,]
File2_data[,-1] = File1_data[,-1]
  ...
  if( method == "GOnly"){ 
    ...
    # Run the script 
    gutman_mcmc_Speed("no",MCMCSeed, its,burnin, thinning, gap, n.RIB, RIB, File1_data, File2_data,K,which.Y2, which.Y1,type1seeds, where.coefs.primary, where.coefs.secondary, p, p2, n.type1,needs.C,reps,PermSampling,secondary.option="dependent", n1, n2,col.Y1, col.Y2,paste("G",namepart,"NewEta",sep=""))
